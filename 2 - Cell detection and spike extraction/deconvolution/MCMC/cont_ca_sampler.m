function [SAMPLES, defparams] = cont_ca_sampler(Y,params)
...
defparams.Nsamples = 100;                   % number of samples after burn in period
...
defparams.gam_step = 20;                    % flag for how often to update time constants 
...
defparams.std_move = 0;                     % standard deviation of spike move kernel
defparams.add_move = ceil(T/100);           % number of add moves
defparams.init = [];                        % sampler initializer
defparams.f = 10.8;                            % imaging rate (irrelevant)
defparams.p = 2;                            % order of AR process (use p = 1 or p = 2)
